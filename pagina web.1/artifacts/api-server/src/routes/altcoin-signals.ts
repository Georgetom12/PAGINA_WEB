/**
 * PSY-ULT2 — Motor de señales altcoins con análisis técnico completo
 * Indicadores: RSI, StochRSI, MACD, EMA Stack, CVD, Volume Spike,
 *              FVG, Liquidity Sweeps, Order Blocks, BOS/CHoCH,
 *              Premium/Discount, S/R Levels, Funding Rate, OI, Fibonacci TP/SL
 */
...
async function fetchCandles(okxId: string, binanceId: string, bar: string, limit = 200): Promise<OHLCV[]> {
  try { const c = await okxCandles(okxId, bar, limit); if (c.length >= 30) return c; } catch { /* OKX falló */ }
  try { const c = await kucoinCandles(okxId, bar, limit); if (c.length >= 30) return c; } catch { /* KuCoin falló */ }
  try { const c = await bybitCandles(binanceId, bar, limit); if (c.length >= 30) return c; } catch { /* todos fallaron */ }
  return [];
}
...
async function genSignal(a: typeof ASSETS[number]): Promise<AltcoinSignal | null> {
  try {
    ...
    if (rsiDiv === "BULLISH_DIV") {
      bullPts += 2 + (divergencia.confianza / 100) * 2;
...
router.get("/altcoin-signals", async (_req: Request, res: Response) => {
  const cached = cGet<{ signals: AltcoinSignal[]; cachedAt: string }>("psyult2_v1");
  if (cached) { res.json(cached); return; }

  try {
    const signals: AltcoinSignal[] = [];
    const assets = [...ASSETS];
    ...
    signals.sort((a, b) => {
      const sv = { FUERTE: 2, MODERADO: 1, DÉBIL: 0 };
      return (sv[b.strength] - sv[a.strength]) || (b.score - a.score);
    });

    const resp = { signals, cachedAt: new Date().toISOString() };
    cSet("psyult2_v1", resp, 10 * 60_000);
    res.json(resp);
  } catch (err) {
    logger.error({ err }, "altcoin-signals error");
