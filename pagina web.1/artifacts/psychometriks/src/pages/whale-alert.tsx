...
function WhaleFeedsTab() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const tgAlerts  = tgRes.status  === "fulfilled" && tgRes.value.ok  ? tgRes.value.alerts  : [];
      const apiAlerts = apiRes.status === "fulfilled" && apiRes.value.ok ? apiRes.value.alerts : [];
                      ...
                      {s.dydx ? (
                        <div className="grid grid-cols-2 gap-1.5">
                          {[
                            ...
                            { label: "FUNDING",  value: (s.dydx.fundingRate * 100).toFixed(5) + "%", color: s.dydx.fundingRate < 0 ? "#00e676" : "#ff6b6b" },
                            { label: "FUNDING APR", value: s.dydx.fundingAnnual.toFixed(2) + "%", color: s.dydx.fundingRate < 0 ? "#00e676" : "#ff6b6b" },
                          ].map(item => (
                    ...
                    <div className="border border-[#ffd70020] bg-[#0a0a00] p-3">
                      ...
                      {s.gmx ? (
                        <div>
                          ...
                          <div className="mt-2">
                            <div className="flex justify-between mb-1">
                              <span className="font-sharetech text-[6px] text-[#00e676]">LONG {s.gmx.longPct.toFixed(1)}%</span>
                              <span className="font-sharetech text-[6px] text-[#ff1744]">SHORT {s.gmx.shortPct.toFixed(1)}%</span>
                            </div>
...
function ShortSqueezeTab() {
  ...
  return (
    <div>
      {/* Explanation banner */}
      <div className="border border-[#e040fb30] bg-[#060010] p-4 mb-5">
        <div className="font-sharetech text-[8px] tracking-[0.2em] text-[#e040fb] mb-2">⚡ RADAR DE SQUEEZE MULTI-EXCHANGE</div>
        <div className="font-space text-[9px] text-[#7ab3c8] mb-4 leading-relaxed">
          Detecta desequilibrios extremos de funding rate. Un <span className="text-[#00e676]">SHORT SQUEEZE</span> ocurre cuando el funding es muy negativo (shorts pagando caro) — cualquier subida fuerza liquidaciones en cadena.
          Un <span className="text-[#ff6b6b]">LONG SQUEEZE</span> es lo opuesto: longs sobrecargados, riesgo de caída forzada.
        </div>
        <div className="grid grid-cols-3 gap-3">
          {[
            ...
            <div key={s.label} className="bg-[#040d18] border border-[#1a2535] p-3 text-center">
              <div className="font-sharetech text-[6px] text-[#5a8898] tracking-[0.08em] mb-1">{s.label}</div>
              <div className="font-bebas text-3xl leading-none" style={{ color: s.color }}>{s.value}</div>
            </div>
          ))}
      ...
      <div className="flex items-center gap-3 mb-5 flex-wrap">
        {["Binance", "OKX", "Bybit", "Gate.io", "MEXC", "KuCoin", "CoinGlass"].map(src => (
          <span key={src} className="font-sharetech text-[7px] px-2.5 py-1 border border-[#0d2030] text-[#7ab3c8]">{src}</span>
        ))}
        <div className="ml-auto flex items-center gap-2">
          <div className="w-2 h-2 rounded-full bg-[#e040fb] animate-pulse" />
          <span className="font-sharetech text-[7px] text-[#e040fb] tracking-[0.15em]">SCAN ACTIVO · 5min</span>
        </div>
      </div>

      {loading ? (
        <div className="p-16 text-center">
          <div className="font-sharetech text-[10px] text-[#e040fb] animate-pulse tracking-[0.25em] mb-3">ESCANEANDO FUNDING RATES…</div>
          <div className="font-space text-[8px] text-[#3a5a6a]">Binance · MEXC · CoinGlass · Multi-Exchange</div>
        </div>
      ) : error ? (
        <div className="p-10 text-center border border-[#ff174430]">
          <div className="font-bebas text-xl text-[#ff1744] mb-2">ERROR DE CONEXIÓN</div>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-4">
          {candidates.map(c => {
            ...
            const barPct = Math.min(100, frAbs / 0.05 * 100); // 0.05% = 100% bar

            return (
              <div key={c.symbol} className="relative overflow-hidden border transition-all hover:border-opacity-70"
                style={{ borderColor: `${typeColor}40`, background: "linear-gradient(160deg,#020b12,#030d16)", boxShadow: c.score >= 4 ? `0 0 25px ${typeColor}15` : "none" }}>
                <div className="h-[2px]" style={{ background: `linear-gradient(90deg,transparent,${typeColor},transparent)` }} />
                <div className="p-4">
                  {/* Header */}
                  <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center gap-3">
                      <span className="font-bebas text-4xl leading-none" style={{ color: c.color }}>{c.icon}</span>
                      <div>
                        <div className="font-bebas text-2xl text-white tracking-wider leading-none">{c.symbol}</div>
                        <div className="font-sharetech text-[7px] text-[#5a8898]">{c.name}</div>
                      </div>
                    </div>
                    <div className="flex flex-col items-end gap-1">
                      <span className="font-sharetech text-[7px] px-2.5 py-1 border animate-pulse"
                      ...
                      <span className="font-sharetech text-[7px]" style={{ color: typeColor }}>
                  ...
                  <div className="grid grid-cols-2 gap-2 mb-4">
                    {[
                      ...
                      <div key={item.label} className="bg-[#040d18] border border-[#0d1a2a] p-2.5 text-center">
                        <div className="font-sharetech text-[6px] text-[#5a8898] mb-1">{item.label}</div>
                        <div className="font-space text-[10px] font-bold" style={{ color: item.color }}>{item.value}</div>
                      </div>
                    ))}
                  </div>
                  ...
                  {c.volume && c.cvd && c.openInterest && (
                    <div className="grid grid-cols-3 gap-2 mb-4">
                      {/* Volumen 24H */}
                      <div className="bg-[#040d18] border border-[#0d1a2a] p-2.5 text-center">
                        <div className="font-sharetech text-[6px] text-[#5a8898] mb-1">VOL 24H</div>
                        <div className="font-space text-[9px] font-bold text-[#e0e6ff]">
                          {c.volume.vol24h > 1000 ? (c.volume.vol24h / 1000).toFixed(1) + "K" : c.volume.vol24h.toFixed(1)} {c.volume.unit}
                        </div>
                        <div className="font-sharetech text-[6px] mt-1"
                      ...
                      {/* CVD */}
                      <div className="bg-[#040d18] border border-[#0d1a2a] p-2.5 text-center">
                        <div className="font-sharetech text-[6px] text-[#5a8898] mb-1">CVD 24H</div>
                        <div className="font-space text-[9px] font-bold"
                          style={{ color: c.cvd.trend === "COMPRAS" ? "#00e676" : "#ff6b6b" }}>
                          {c.cvd.trend}
                        </div>
                        <div className="font-sharetech text-[6px] text-[#5a8898] mt-1">{c.cvd.dominancePct}% dom.</div>
                      </div>
                      {/* OI */}
                      <div className="bg-[#040d18] border border-[#0d1a2a] p-2.5 text-center">
                        <div className="font-sharetech text-[6px] text-[#5a8898] mb-1">OPEN INT.</div>
                        <div className="font-space text-[9px] font-bold text-[#e0e6ff]">
                  ...
                  {c.confirmations != null && (
                    <div className="flex items-center gap-2 mb-4 px-2">
                      <span className="font-sharetech text-[6px] text-[#5a8898]">CONFIRMACIONES</span>
                      <div className="flex gap-1 flex-1">
                        {[0,1,2,3].map(i => (
                          <div key={i} className="h-1.5 flex-1 rounded-sm transition-all"
                            style={{ background: i < c.confirmations ? "#00e676" : "#0d1a2a" }} />
                        ))}
                      </div>
                      <span className="font-space text-[8px] font-bold"
                  ...
                  <div className="border p-3 mb-4" style={{ borderColor: `${typeColor}25`, background: `${typeColor}08` }}>
                    <div className="flex items-center justify-between mb-2">
                      <span className="font-sharetech text-[7px]" style={{ color: typeColor }}>
                        {isSS ? "⚡ INTENSIDAD SQUEEZE CORTOS" : isLS ? "💥 INTENSIDAD SQUEEZE LARGOS" : "📊 PRESIÓN DE FUNDING"}
                      </span>
                      <span className="font-bebas text-lg leading-none" style={{ color: riskColor }}>{c.score}/5</span>
                    </div>
                    <div className="h-2.5 bg-[#0a1520] rounded-full overflow-hidden">
                      <div className="h-full rounded-full transition-all duration-700"
                    ...
                    <div className="flex justify-between mt-1">
                      <span className="font-sharetech text-[6px] text-[#5a8898]">NEUTRO</span>
                      <span className="font-sharetech text-[6px]" style={{ color: riskColor }}>{barPct.toFixed(0)}%</span>
                      <span className="font-sharetech text-[6px] text-[#ff1744]">EXTREMO</span>
                    </div>
                  </div>

                  {/* What it means */}
                  <div className="bg-[#030c14] border border-[#0d1a2a] p-2.5 mb-3">
                    <div className="font-sharetech text-[6px] text-[#5a8898] mb-1">📖 QUÉ SIGNIFICA</div>
                    <div className="font-space text-[7px] text-[#7ab3c8] leading-relaxed">
                      {isSS
                  ...
                  {/* Sources breakdown */}
                  {c.sources.length > 0 && (
                    <div className="border border-[#0d1a2a] p-2.5">
                      <div className="font-sharetech text-[6px] text-[#5a8898] tracking-[0.1em] mb-2">FUENTES ({c.sources.length} exchanges)</div>
                      <div className="grid grid-cols-2 gap-1.5">
                        {c.sources.map((src, i) => (
                          <div key={i} className="flex items-center justify-between bg-[#040d18] px-2 py-1">
                            <span className="font-sharetech text-[6px] text-[#5a8898] truncate">{src.exchange}</span>
                            <span className="font-space text-[7px] font-bold ml-2" style={{ color: parseFloat(src.rate) < 0 ? "#00e676" : parseFloat(src.rate) > 0 ? "#ff6b6b" : "#7ab3c8" }}>{src.rate}</span>
                          </div>
                        ))}
                      </div>
                    </div>
                  ...
                  <div className="flex items-start justify-between mb-3">
                    ...
                    <div className="text-right">
                      ...
                      <div className="font-space text-[9px] font-bold mt-0.5"
                        ...
                        {gem.priceChange24h>=0?"▲":""}{gem.priceChange24h.toFixed(1)}%
                      </div>
                    </div>
                  </div>
...
function OracleFeedsTab() {
  ...
  const tickerItems = data?.ticker ?? ["PSYCHOMETRIKS // ORACLE FEEDS · WHALE INTELLIGENCE DASHBOARD · TIEMPO REAL"];
