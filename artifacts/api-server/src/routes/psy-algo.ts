...
function stochRsi(closes: number[], rsiP = 14, stochP = 14, smoothK = 3): { k: number; d: number } {
  ...
  for (let i = stochP - 1; i < rsis.length; i++) {
    ...
    kArr.push(mx === mn ? 50 : ((rsis[i]! - mn) / (mx - mn)) * 100);
  }
  const k = kArr.slice(-smoothK).reduce((a, b) => a + b, 0) / Math.min(smoothK, kArr.length);
  const d = kArr.slice(-(smoothK * 2), -smoothK).reduce((a, b) => a + b, 0) / smoothK || k;
...
function detectSR(candles: OHLCV[], price: number): "SUPPORT" | "RESISTANCE" | null {
  const { highs, lows } = swings(candles, 4);
  const recentH = highs.slice(-5).map(h => h.price).filter(h => h > price);
  const recentL = lows.slice(-5).map(l => l.price).filter(l => l < price);
  const nearRes = recentH.length && (Math.min(...recentH) / price - 1) < 0.015;
  const nearSup = recentL.length && (1 - Math.max(...recentL) / price) < 0.015;
  if (nearSup) return "SUPPORT";
  if (nearRes) return "RESISTANCE";
  return null;
}
...
function detectBOS(candles: OHLCV[]): { bullBOS: boolean; bearBOS: boolean; bullCHoCH: boolean; bearCHoCH: boolean } {
  ...
  const st = structure(candles.slice(-20));
  let bullBOS = false, bearBOS = false, bullCHoCH = false, bearCHoCH = false;
  if (highs.length >= 2) {
    const prevH = highs.at(-2)!.price;
    if (price > prevH) { bullBOS = true; if (st === "BEARISH") bullCHoCH = true; }
  }
  if (lows.length >= 2) {
    const prevL = lows.at(-2)!.price;
    if (price < prevL) { bearBOS = true; if (st === "BULLISH") bearCHoCH = true; }
  }
...
async function genSignal(a: typeof ASSETS[number]) {
  ...
  const fvg     = detectFVG(c4h.slice(-30));
  const sweeps  = detectSweeps(c4h.slice(-15), swH, swLow);
  const nearSR  = detectSR(c4h, price);
  const bos     = detectBOS(c4h.slice(-40));
  ...
  // StochRSI (max +2)
  if (stoch.k < 15)      { score += 2; dir = "LONG";  inds.push(`StochRSI K=${stoch.k} — HIPERSOBREVENTA extrema ▲`); }
  else if (stoch.k < 25) { score += 1;                inds.push(`StochRSI K=${stoch.k} — sobreventa ▲`); }
  if (stoch.k > 85)      { score += 2; dir = "SHORT"; inds.push(`StochRSI K=${stoch.k} — HIPERSOBRECOMPRA extrema ▼`); }
  else if (stoch.k > 75) { score += 1;                inds.push(`StochRSI K=${stoch.k} — sobrecompra ▼`); }

  // FVG (max +2)
  if (fvg.bullish)  { score += 2; inds.push("Fair Value Gap ALCISTA 4H — imán de precio arriba ▲"); }
  if (fvg.bearish)  { score += 2; inds.push("Fair Value Gap BAJISTA 4H — imán de precio abajo ▼"); }

  // Liquidity Sweeps (max +3)
  if (sweeps.bullSweep) { score += 3; dir = "LONG";  inds.push("Liquidity SWEEP alcista 4H — barrido de stops SHORT ▲"); }
  if (sweeps.bearSweep) { score += 3; dir = "SHORT"; inds.push("Liquidity SWEEP bajista 4H — barrido de stops LONG ▼"); }

  // S/R proximity (max +2)
  if (nearSR === "SUPPORT")    { score += 2; inds.push("Precio en SOPORTE clave 4H — zona institucional de compra ▲"); }
  if (nearSR === "RESISTANCE") { score += 2; inds.push("Precio en RESISTENCIA clave 4H — zona institucional de venta ▼"); }

  // BOS / CHoCH (max +2)
  if (bos.bullBOS || bos.bullCHoCH) { score += 2; dir = "LONG";  inds.push(bos.bullCHoCH ? "CHoCH ALCISTA 4H — cambio de carácter ▲" : "BOS ALCISTA 4H — ruptura de estructura ▲"); }
  if (bos.bearBOS || bos.bearCHoCH) { score += 2; dir = "SHORT"; inds.push(bos.bearCHoCH ? "CHoCH BAJISTA 4H — cambio de carácter ▼" : "BOS BAJISTA 4H — ruptura de estructura ▼"); }

  const scoreNorm = Math.round(Math.min(100, (score / 37) * 100));
  ...
  // Entry / TP / SL
  const rng = swH - swLow;
  let tp1 = price * 1.05, tp2 = price * 1.10, tp3 = price * 1.15, sl = price * 0.95;
  if (dir === "LONG") {
  ...
  return {
    ...
    entry: fmt2(price), tp1: fmt2(tp1), tp2: fmt2(tp2), tp3: fmt2(tp3), sl: fmt2(sl),
    rr: `1:${rrNum.toFixed(2)}`,
    currentPrice: fmt2(price),
    indicators: {
      rsi1h: rsi1h.toFixed(1), rsi4h: rsi4h.toFixed(1), rsiDivergence: rsiDiv,
      macdCross: macdX, macdHistogram: mH, cvdTrend,
      volumeSpike: volSpike.toFixed(2), struct4h: st4h, structWeekly: stW,
    ...
    fibonacci: { swingHigh: fmt2(swH), swingLow: fmt2(swLow), fib382: fmt2(fibs.f382), fib500: fmt2(fibs.f500), fib618: fmt2(fibs.f618), fib786: fmt2(fibs.f786), fib1272: fmt2(fibs.f1272), fib1618: fmt2(swH + rng * 0.618) },
