...
// ── Real signal type (matches /api/altcoin-signals response) ──────────────────
interface WhaleSignal {
  ...
  macdHistogram: number; macdCross: boolean;
  volumeSpike: number;
  htfBias?: "BULLISH" | "BEARISH" | "NEUTRAL";
  struct1h?: string; struct4h?: string;
  atr1h?: number;
  indicators: string[];
                  ...
                  <div key={sig.symbol} className="relative overflow-hidden border bg-[#030a14] hover:-translate-y-0.5 transition-all duration-300"
                    ...
                    <div className="p-5">
                      ...
                      <div className="grid grid-cols-4 gap-2 mb-3">
                        {[
                          { label: "ENTRADA", value: sig.entry, color: "#ffffff",  bg: "#0d1520" },
                          { label: "TP1",     value: sig.tp1,   color: "#00e676",  bg: "#001a0a" },
                          { label: "TP2",     value: sig.tp2,   color: "#00ff88",  bg: "#001a0a" },
                          { label: "STOP",    value: sig.sl,    color: "#ff1744",  bg: "#1a0005" },
                        ].map(item => (
                          <div key={item.label} className="text-center border py-2"
                            style={{background:item.bg, borderColor:`${item.color}20`}}>
                            <div className="font-sharetech text-[7px] text-[#7ab3c8] tracking-[0.1em] mb-0.5">{item.label}</div>
                      ...
                      <div className="grid grid-cols-4 gap-1.5 mb-3 p-2.5 bg-[#040d18] border border-[#0d1a2a]">
                        {[
                          {label:"RSI 1H",    value:String(sig.rsi1h),  color:sig.rsi1h<35?"#00e676":sig.rsi1h>65?"#ff1744":"#ffd700"},
                          {label:"MACD",      value:sig.macdCross?"✕ CRUCE":sig.macdHistogram>0?"▲ POS":"▼ NEG", color:sig.macdHistogram>0?"#00e676":"#ff1744"},
                          {label:"VOL SPIKE", value:`×${sig.volumeSpike}`, color:sig.volumeSpike>2?"#00e5ff":"#7ab3c8"},
                          ...
                          <div key={ind.label} className="text-center">
                            <div className="font-sharetech text-[6px] text-[#5a8898] tracking-[0.12em] mb-0.5">{ind.label}</div>
                            <div className="font-space text-[10px] font-bold" style={{color:ind.color}}>{ind.value}</div>
                          </div>
                        ))}
                      </div>
                      ...
                      <div className="flex flex-wrap gap-1 mb-3">
                        {sig.indicators.slice(0, 3).map((ind, i) => (
                          <span key={i} className="font-sharetech text-[7px] px-2 py-0.5"
                      ...
                      {/* Footer */}
                      <div className="flex items-center justify-between pt-2.5 border-t border-[#0d1a2a]">
                        <div className="flex items-center gap-2">
                          <span className="font-bebas text-lg text-white tracking-wider leading-none">R/R</span>
                        ...
                        <div className="flex items-center gap-1.5">
                          ...
                          <span className="font-sharetech text-[8px] text-[#7ab3c8] ml-1">{sig.score}/8</span>
                        </div>
                      </div>
                    </div>
                  </div>
