...
const CHIPS_FALLBACK = ["AAPL", "MSFT", "NVDA", "TSLA", "EURUSD=X", "GBPUSD=X", "USDJPY=X", "AUDUSD=X"];
    ...
    <div className="min-h-screen bg-[#020408] text-white font-rajdhani flex flex-col">
      ...
      <div className="flex-1 flex items-center justify-center px-6 pt-20">
        <div className="border border-[#1a2535] bg-[#060a0f] p-10 max-w-md w-full text-center">
          ...
          <div className="font-bebas text-3xl text-white mb-2">PSYCHOMETRIKS IA TRADING — BOLSA DE VALORES</div>
...
export default function IntelligentAiTrading() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const r = await fetch("/api/ia-trading-bolsa/top-symbols", { headers: { "X-PSY-Token": auth?.token ?? "" } });
  ...
  async function analizar(sym?: string) {
    ...
    setData(null);
    try {
      const r = await fetch("/api/ia-trading-bolsa/analizar", {
        method: "POST",
    ...
    <div className="min-h-screen bg-[#020408] text-white font-rajdhani">
      ...
      <div className="max-w-6xl mx-auto px-4 pt-24 pb-16">
        ...
        <div className="mb-8">
          ...
          <h1 className="font-bebas text-4xl md:text-5xl text-white">PSYCHOMETRIKS IA TRADING — BOLSA DE VALORES</h1>
        ...
        <div className="border border-[#1a2535] bg-[#060a0f] p-4 mb-6">
          ...
          <p className="text-[13px] text-white/90 leading-relaxed mb-3">
            EMA (9/21 en 1H, 50/200 en 4H) · RSI en 5 temporalidades (15m a 1 semana) con detección de divergencias reales
            (por puntos de giro + volumen + momentum, no solo RSI) · MACD (4H) · ATR (4H) · estructura de mercado (4H, 1D, 1 semana) ·
            ...
            <br /><span className="text-[#5a6b7d]">Nota: acciones y forex no tienen futuros perpetuos, así que aquí no hay Open Interest, CVD ni Funding Rate — esas métricas son exclusivas del motor cripto.</span>
          </p>
          <div className="bg-[#0a0f16] border-l-2 border-[#00e5ff] p-3 text-[13px]">
            <span className="text-[#00e5ff] font-bold">📈 Temporalidades usadas:</span>{" "}
            <span className="text-white/80">15m, 1H, 4H, 1D y 1 semana — pensado para posiciones de horas a semanas, no para scalping de minutos (acciones/forex no tienen la profundidad de datos intradía que sí tiene cripto).</span>
          </div>
        </div>
