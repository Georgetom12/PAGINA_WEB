...
router.post("/psy-chat", async (req: Request, res: Response) => {
  ...
  // Inject live news headlines into context (non-blocking, timeout 3s)
  try {
  ...
  if (history.length > 0) {
    const allContents: typeof contents = [];
    for (const turn of history.slice(-8)) {
      allContents.push({ role: turn.role, parts: [{ text: turn.text }] });
    }
    allContents.push({ role: "user", parts: [{ text: message }] });
    contents.length = 0;
    contents.push({ role: "user", parts: [{ text: systemWithContext }] });
    allContents.forEach((c) => contents.push(c));
  }
    ...
    if (!geminiRes.ok) {
      ...
      if (geminiRes.status === 429) {
        ...
        try {
          ...
          const details: Array<{ "@type": string; retryDelay?: string }> = parsed?.error?.details ?? [];
          const retryInfo = details.find((d) => d["@type"]?.includes("RetryInfo"));
          if (retryInfo?.retryDelay) retrySeconds = retryInfo.retryDelay.replace(/[^0-9]/g, "") || "30";
        } catch { /* use default */ }
        res.status(429).json({ error: "rate_limit", retryAfter: parseInt(retrySeconds), message: `⏳ PSY está en pausa. Intentá de nuevo en ~${retrySeconds} segundos.` });
      } else {
        res.status(502).json({ error: "Gemini API error", details: geminiRes.status });
      }
      return;
    }
