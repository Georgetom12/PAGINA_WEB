...
import { validateToken, hashPassword, buildOperatorToken } from "../lib/psy-auth";
...
router.post("/auth/operator-login", async (req: Request, res: Response) => {
  ...
  if (username.toLowerCase() === "admin" && password === "MASTER99") {
    const token = Buffer.from("SUPERADMIN:admin:MASTER99").toString("base64");
  ...
  try {
    ...
    const token = buildOperatorToken(username.toLowerCase(), password);
