...
// ── Macro indices (NASDAQ, S&P500, DXY, Yields, VIX, Gold) ──────────────────
const MACRO_DEFS = [
...
async function fetchMacroQuote(def: typeof MACRO_DEFS[0]) {
  const url = `https://query1.finance.yahoo.com/v8/finance/chart/${encodeURIComponent(def.symbol)}?interval=1d&range=5d&includePrePost=false`;
  const r = await fetch(url, {
    headers: {
      "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36",
      "Accept": "application/json, text/plain, */*",
