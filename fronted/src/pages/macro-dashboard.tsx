...
export default function MacroDashboard() {
  ...
  const [fredKpis, setFredKpis] = useState<{ fedfunds: number | null; cpi: number | null; m2: number | null; t10y2y: number | null }>({ fedfunds: null, cpi: null, m2: null, t10y2y: null });
  ...
  // ── FRED real-time macro indicators ──────────────────────────────────────
  useEffect(() => {
    const fetchFred = async (id: string) => {
      try {
        const r = await fetch(`/api/proxy/fred/series?id=${id}&limit=1&sort=desc`);
        if (!r.ok) return null;
        const d = await r.json() as { ok: boolean; observations: { date: string; value: number }[] };
        return d.ok && d.observations.length > 0 ? d.observations[0].value : null;
      } catch { return null; }
    };
    Promise.all([
      fetchFred("FEDFUNDS"),
      ...
      fetchFred("M2SL"),
      fetchFred("T10Y2Y"),
    ]).then(([fedfunds, cpi, m2, t10y2y]) => {
      setFredKpis({ fedfunds, cpi, m2, t10y2y });
    });
  }, []);
  ...
  return (
    <div className="min-h-screen bg-[#020408] text-white font-rajdhani">
      ...
      {/* FRED Live Data strip */}
      <section className="px-6 md:px-12 pb-4">
        <div className="border border-[#e040fb22] bg-[#060a0f] p-4">
          <div className="flex items-center gap-3 mb-3">
            <div className="w-1.5 h-1.5 rounded-full bg-[#e040fb] animate-pulse" />
            <span className="font-sharetech text-[9px] tracking-[0.3em] text-[#e040fb]">FRED · FEDERAL RESERVE ST. LOUIS · DATOS REALES</span>
          </div>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {[
              { label: "FED FUNDS RATE", value: fredKpis.fedfunds !== null ? `${fredKpis.fedfunds.toFixed(2)}%` : "—", sub: "Tasa objetivo Fed", color: fredKpis.fedfunds !== null && fredKpis.fedfunds > 4 ? "#ff6d00" : "#00e676" },
              { label: "CPI (INFLACIÓN)", value: fredKpis.cpi !== null ? fredKpis.cpi.toFixed(1) : "—", sub: "CPIAUCSL índice", color: "#ffd700" },
              { label: "M2 MONEY SUPPLY", value: fredKpis.m2 !== null ? `$${(fredKpis.m2 / 1000).toFixed(1)}T` : "—", sub: "Masa monetaria USD", color: "#00e5ff" },
              { label: "YIELD SPREAD", value: fredKpis.t10y2y !== null ? `${fredKpis.t10y2y > 0 ? "+" : ""}${fredKpis.t10y2y.toFixed(2)}%` : "—", sub: fredKpis.t10y2y !== null ? (fredKpis.t10y2y < 0 ? "⚠ Curva invertida" : "Curva normal") : "10Y − 2Y spread", color: fredKpis.t10y2y !== null && fredKpis.t10y2y < 0 ? "#ff6d00" : "#00e676" },
            ].map(k => (
              <div key={k.label}>
                <div className="font-sharetech text-[8px] tracking-[0.15em] text-[#5a8898] mb-1">{k.label}</div>
                <div className="font-bebas text-2xl" style={{ color: k.color }}>
                  {k.value === "—" ? <span className="text-[#2a4a5a] text-sm animate-pulse">CARGANDO...</span> : k.value}
                </div>
                <div className="font-space text-[9px] text-[#4a6070]">{k.sub}</div>
              </div>
            ))}
          </div>
        </div>
