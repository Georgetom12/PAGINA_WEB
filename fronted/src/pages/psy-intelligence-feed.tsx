...
export default function PsyIntelligenceFeed() {
  ...
  useEffect(() => {
    const loadNews = () => {
      setNewsLoading(true);
      fetch("/api/proxy/news?q=bitcoin+crypto+ethereum+blockchain&pageSize=12&lang=es")
        .then(r => r.json())
        .then((d: { ok: boolean; articles: NewsArticle[] }) => {
          if (d.ok && d.articles?.length) setNewsItems(d.articles);
        })
        .catch(() => {})
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 pt-24 pb-16">
        ...
        {loading ? (
          ...
          <>
            ...
            {/* ── NewsAPI Live Feed ── */}
            <div className="border border-[#22d4f522] bg-[#060a0f] p-5 mt-6">
              <div className="flex items-center justify-between mb-5">
                <div>
                  <div className="flex items-center gap-2 mb-1">
                    <div className={`w-1.5 h-1.5 rounded-full ${newsLoading ? "bg-[#ffd700] animate-pulse" : "bg-[#22d4f5]"}`} />
                    <span className="font-sharetech text-[8px] tracking-[0.3em] text-[#22d4f5]">
                      {newsLoading ? "ACTUALIZANDO..." : newsItems.length > 0 ? `NEWSAPI LIVE · ${newsItems.length} ARTÍCULOS` : "NEWSAPI"}
                    </span>
                  </div>
                  <div className="font-bebas text-xl text-white">NOTICIAS CRYPTO</div>
                  <p className="font-space text-[9px] text-[#5a8898]">Bitcoin · Ethereum · Blockchain · en español · actualiza cada 15 min</p>
                </div>
              </div>
              {newsItems.length === 0 && !newsLoading ? (
                <div className="font-space text-[9px] text-[#5a8898] text-center py-8">Cargando noticias...</div>
              ) : newsLoading && newsItems.length === 0 ? (
                <div className="space-y-3">
                  {[...Array(4)].map((_, i) => (
                    <div key={i} className="h-14 bg-[#0d1520] animate-pulse rounded" />
                  ))}
                </div>
              ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  {newsItems.map((article, i) => (
                    <a key={i} href={article.url} target="_blank" rel="noopener noreferrer"
                      className="block border border-[#0d2030] bg-[#040f18] p-3 no-underline hover:border-[#22d4f540] transition-colors">
                      <div className="flex items-start gap-3">
                        <div className="w-6 h-6 rounded shrink-0 flex items-center justify-center bg-[#22d4f508] border border-[#22d4f520] text-[0.75rem]">📰</div>
                        <div className="flex-1 min-w-0">
                          <div className="font-space text-[11px] text-[#dceaf5] leading-snug mb-1.5 line-clamp-2">{article.title}</div>
                          <div className="flex gap-2 items-center">
                            <span className="font-sharetech text-[8px] text-[#22d4f5] tracking-[0.05em]">{article.source}</span>
                            <span className="font-space text-[8px] text-[#3d6480]">
                              {new Date(article.publishedAt).toLocaleDateString("es-EC", { day: "2-digit", month: "short", hour: "2-digit", minute: "2-digit" })}
                            </span>
                          </div>
                          {article.description && (
                            <div className="font-space text-[9px] text-[#5a7080] mt-1 line-clamp-1">{article.description}</div>
                          )}
                        </div>
                        <span className="text-[#3d6480] text-[0.7rem] shrink-0">→</span>
                      </div>
                    </a>
                  ))}
                </div>
